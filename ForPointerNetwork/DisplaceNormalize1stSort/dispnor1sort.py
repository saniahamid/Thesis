...
This code reads in the data of the matching, displaces it, normalizes it using the 1st method and sorts it. 
"""
...
file_name = "matching_30.txt"
fo = open("matching_30_dns","w")
...
def normalize1(inputs,outputs):
    ...
    for i in range(inputs.shape[0]):
        for j in range(0,inputs.shape[1]-1,2):
            inputs_norX.append(inputs[i][j])
            inputs_norY.append(inputs[i][j+1])
        
    inputs_norX = np.asarray(inputs_norX)
    inputs_norY = np.asarray(inputs_norY)
    
    min_X = np.amin(inputs_norX)
    max_X = np.amax(inputs_norX)
    min_Y = np.amin(inputs_norY)
    max_Y = np.amax(inputs_norY)
    
    for i in range(inputs.shape[0]):
        for j in range(0, inputs.shape[1]-1,2):
            inputs[i][j] = (inputs[i][j] - min_X) / (max_X - min_X)
            inputs[i][j+1] = (inputs[i][j+1] -  min_Y) / (max_Y - min_Y)
    
#    for k in range(0,inputs.shape[1]-1,2):
#        inputs[i][k] = float("{0:.6f}".format((inputs[i][k])))
#        inputs[i][k+1] = float("{0:.6f}".format((inputs[i][k+1])))
...
def sort(inputs,outputs):
    ...
    for i in range(inputs.shape[0]):
        ...
        inputs_sort = sorted(inputs_sort_temp, key = lambda x : x[1])
        l = 0
